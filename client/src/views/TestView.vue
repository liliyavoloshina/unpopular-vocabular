<template>
  <section class="section">
      ...
      <div class="columns is-centered mt-4">
        <div class="column is-half">
          <b-field label="Choose difficulty:">
            <b-select v-model="difficulty" expanded>
          ...
          <b-field label="Сhoose the number of words">
            <b-numberinput v-model="number" :min="1" :max="maxLength" class="has-addons-centered" controls-position="compact" type="is-dark" expanded />
          </b-field>
          <b-button class="is-size-5 mt-5" type="is-primary" expanded @click="startTest">
            Start!
          </b-button>
        </div>
      </div>
    ...
    <test-form v-else :words="selectedWords" @tryAgain="startTest" />
...
export default {
  ...
  computed: {
    ...
    selectedWords() {
      return this.words.filter(word => word.popularity === this.difficulty).slice(0, this.number)
    }
  },
  async created() {
    this.isLoading = true
